...
#tl_admin_users = ['nautaii', 'edel1989', 'Luis_Daniel_Diaz', '
', 'edel1989', 'Rassr8', 'EnrikeTamaki', 'Rebelde59', '', 'joCanG3l', 'adryes95', 'EL_Wizard'] #Poner aqui los user con acceso permanente
tl_admin_users = ['nautaii', 'Stvz20']
godlist = ['nautaii','adryes95'] #Poner aqui los admin 
...
async def send_root(bot,ev,username):
    ...
    reply = f'👤Sesión {username} 🗂️({len(listdir)} archivos) 📄\n\n'
    i=-1
    for item in listdir:
        i+=1
        fname = item
...
async def download_progress(dl, filename, currentBits, totalBits, speed, totaltime, args):
    try:
        ...
        if True:
            ...
            msg += '☑📀Size: ' + sizeof_fmt(currentBits) + ' -- ' + sizeof_fmt(totalBits) + '\n'
            #msg += '↩️ Descargado: ' + sizeof_fmt(currentBits) + '\n'
            msg += '🚀Speed: ' + sizeof_fmt(speed) + '/s | ETA: ' + str(time.strftime('%H:%M:%S', time.gmtime(totaltime))) + 's\n'
            #msg += '\n' + text_progres(currentBits, totalBits) + ' ' + str(porcent(currentBits, totalBits)) + '%\n' + '\n'
...
async def compress(bot,ev,text,message,username):
        ...
        if index != None:
            ...
            while index<range:
                ffullpath = config.ROOT_PATH + username + '/' + listdir[index]
                await bot.edit_message(ev.chat,message,text=f'📚 {listdir[index]} 📚 - Comprimido✅ /ls')
